 ...
 */
  ...
  student = await tryCatch(Student.findOne({ regNo: regNo }));
...
const addStudentForAdvisor = async (req, res) => {
  ...
  if (!req.body.regNo || !req.body.name || !req.body.dob || !req.body.password)
    return res.status(400).json({ eMessage: "need regNo name dob password" });
...
const addStudentForAdmin = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ eMessage: "need regNo name dob password batch degree section" });
